...
function PluginInfo(dirname) {
    ...
    function _parseJsModule(tag) {
        var ret = {
            ...
            src: tag.attrib.src,
            clobbers: tag.findall('clobbers').map(function (tag) {
                return {target: tag.attrib.target};
            }),
            merges: tag.findall('merges').map(function (tag) {
                return {target: tag.attrib.target};
            }),
    ...
    if (self.keywords) {
        self.keywords = self.keywords.split(',').map(function (s) {
            return s.trim();
        });
    }
